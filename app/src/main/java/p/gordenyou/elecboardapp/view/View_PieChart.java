...
public class View_PieChart extends DemoView implements Runnable {
    ...
    public View_PieChart(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void chartRender() {
        try {
            ...
            ltrb[2] = DensityUtil.dip2px(getContext(), 70); //right
    ...
    public void SetChartDate(LinkedList<PieData> chartData){
    ...
    private void chartDataSet() {
        ...
        //设置图表数据源
        PieData pieData = new PieData("自制件", "自制件:15%", 15, Color.rgb(77, 83, 97));
//        pieData.setCustLabelStyle(XEnum.SliceLabelStyle.OUTSIDE, Color.WHITE);

        //pieData.setItemLabelRotateAngle(rotateAngle)

        chartData.add(pieData);

        chartData.add(new PieData("采购件", "采购件(5%)", 5, Color.rgb(75, 132, 1)));

        //将此比例块突出显示
        PieData pd = new PieData("委外件", "委外件(35%)", 35, Color.rgb(180, 205, 230));
        pd.setItemLabelRotateAngle(45.f);
        chartData.add(pd);

        PieData pdOther = new PieData("其它", "其它(15%)", 15, Color.rgb(148, 159, 181));
//        pdOther.setCustLabelStyle(XEnum.SliceLabelStyle.INSIDE, Color.BLACK);
        chartData.add(pdOther);

        PieData pdTea = new PieData("虚拟件", "虚拟件(30%)", 30, Color.rgb(253, 180, 90), true);
